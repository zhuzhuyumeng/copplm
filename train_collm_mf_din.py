...
def parse_args():
    ...
    parser.add_argument("--cfg-path", default='train_configs/collm_pretrain_mf_ood.yaml', help="path to configuration file.")
    parser.add_argument(
        "--options",
        nargs="+",
...
def main():
    ...
    # --- [!!! 步骤 1.1: 计算流行度 !!!] ---
    print("正在计算全局流行度以进行偏见评估...")
    item_counts = train_['iid'].value_counts()
    item_pop_dict = (np.log(item_counts + 1)).to_dict()
    total_item_num = int(item_num)
    print(f"已计算 {len(item_pop_dict)} 个物品的流行度。总物品数为 {total_item_num}。")

    # --- [!!! 步骤 1.2: 直接注入流行度数据到 Task !!!] ---
    # 我们不调用 task.before_evaluation()，因为它会被 runner 覆盖。
    # 我们直接在 task 实例上设置持久化属性。
    # (my_rec_base_task_ndcg.py 中的新 before_evaluation 会保护这些值)
    print("正在将流行度数据注入评估任务...")
    task.item_pop_dict = item_pop_dict
    task.total_item_num = total_item_num
    task.k_for_bias = 10
    # --- [!!! 修改结束 !!!] ---
