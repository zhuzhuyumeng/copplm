...
#     pos_num = label.sum()
#     if pos_num == 0 or pos_num == label.shape[-1]:
#         return -1
#     ranked_id = torch.argsort(predict)
#     ranked_label = label[ranked_id]
#     flag = 1.0 / torch.log(torch.range(ranked_label.shape[-1])+2.0)
...
def compute_dcg(user, predict, label):
    ...
    for ui,pre_and_true in candidates_dict.items():
        ...
        ui_auc = u_dcg(pre_i,label_i)
        # roc_auc_score(label_i,pre_i)
        if ui_auc >=0:
            auc.append(ui_auc)
    ...
    def evaluation(self, model, data_loaders, cuda_enabled=True):
        ...
        for data_loader in data_loaders.loaders:
            ...
            if is_dist_avail_and_initialized():
                ...
                uauc, _, _ = compute_dcg(users_a,results_logits_a,labels_a)
                print("finished comput auc.....")
            else:
                auc = roc_auc_score(labels_.cpu().numpy(), results_logits_.cpu().numpy())
                uauc = compute_dcg(users_.cpu().numpy(), results_logits_.cpu().numpy(), labels_.cpu().numpy())
