...
class MoiveOOData(RecBaseDataset):
    def __init__(self, text_processor=None, ann_paths=None):
        ...
        self.popular_items = self.item_counts.index[:int(self.item_num * 0.2)].tolist()  # 前20%热门
        self.tail_items = self.item_counts.index[int(self.item_num * 0.2):].tolist()  # 后80%长尾

        # 定义流行度等级 (用于 Prompt)
        # 0: Cold, 1: Normal, 2: Hot
        self.id2pop_level = {iid: (2 if iid in self.popular_items else 1) for iid in self.item_counts.index}
    ...
    def __getitem__(self, index):
        ...
        if self.use_his:
            ...
            pop_desc_map = {0: "rare", 1: "common", 2: "highly popular"}
            pos_pop_desc = pop_desc_map.get(self.id2pop_level.get(pos_item_id, 0), "rare")
            neg_pop_desc = "highly popular"  # 热门负样本肯定是 popular
            neg_tail_desc = "rare"  # 长尾负样本
            one_sample = {
                "UserID": ann['UserID'],
                "InteractedItemIDs_pad": np.array(b),
                # "InteractedItemIDs": ann['InteractedItemIDs'],
