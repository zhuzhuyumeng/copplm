...
def to_py_obj(obj):
    """递归地把 np.bool_/np.int*/np.float*/ndarray 转成 Python 原生类型"""
    if isinstance(obj, dict):
        return {k: to_py_obj(v) for k, v in obj.items()}
    if isinstance(obj, (list, tuple)):
        return [to_py_obj(v) for v in obj]
    if isinstance(obj, np.ndarray):          # <-- 新增
        return obj.tolist()
    if isinstance(obj, np.generic):          # 标量
        return obj.item()
    return obj
    ...
    def log_stats(self, stats, split_name):
        if isinstance(stats, dict):
            ...
            with open(os.path.join(self.output_dir, "log.txt"), "a") as f:
                ...
                f.write(json.dumps(to_py_obj(log_stats)) + "\n")
